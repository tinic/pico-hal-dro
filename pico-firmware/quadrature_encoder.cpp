...
std::expected<int64_t, QuadratureEncoder::EncoderError> 
QuadratureEncoder::get_count(size_t encoder_idx) const noexcept {
    if (!initialized) {
        return std::unexpected(EncoderError::NotInitialized);
    }
...
std::expected<void, QuadratureEncoder::EncoderError> 
QuadratureEncoder::reset_count(size_t encoder_idx) noexcept {
    if (!initialized) {
        return std::unexpected(EncoderError::NotInitialized);
    }
    if (encoder_idx >= kNumEncoders) {
